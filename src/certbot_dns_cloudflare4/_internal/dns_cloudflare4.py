...
class Authenticator(dns_common.DNSAuthenticator):
    ...
    description = (
        "Obtain certificates using a DNS TXT record (if you are using Cloudflare for "
        "DNS)."
    )
    ttl = 120
        ...
        if token:
            if email or key:
                raise errors.PluginError(
                    "{}: dns_cloudflare4_email and dns_cloudflare4_api_key are "
                    "not needed when using an API Token".format(
            ...
            if not email:
                raise errors.PluginError(
                    "{}: dns_cloudflare4_email is required when using a Global "
                    "API Key. (should be email address associated with "
                    "Cloudflare account)".format(credentials.confobj.filename)
                )
            if not key:
                raise errors.PluginError(
                    "{}: dns_cloudflare4_api_key is required when using a "
                    "Global API Key. (see {})".format(
                        credentials.confobj.filename, ACCOUNT_URL
                    )
                )
        else:
            raise errors.PluginError(
                "{}: Either dns_cloudflare4_api_token (recommended), or "
    ...
    def _perform(self, domain: str, validation_name: str, validation: str) -> None:
        self._get_cloudflare_client().add_txt_record(
            domain, validation_name, validation, self.ttl
        )

    def _cleanup(self, domain: str, validation_name: str, validation: str) -> None:
        self._get_cloudflare_client().del_txt_record(
...
class _CloudflareClient:
    ...
    def add_txt_record(
        self,
        domain: str,
        record_name: record_list_params.Name,
        record_content: record_list_params.Content,
        record_ttl: int,
    ) -> None:
        """
        ...
        try:
            ...
            logger.error(
                "Encountered CloudFlareAPIError adding TXT record: %d %s", e, e
            )
            raise errors.PluginError(
                "Error communicating with the Cloudflare API: {0}{1}".format(
                    e, " ({0})".format(hint) if hint else ""
                )
            )
        ...
        if zone_id:
            ...
            if record_id:
                try:
                    ...
                    logger.warning(
                        "Encountered CloudFlareAPIError deleting TXT record: %s", e
                    )
            else:
    ...
    def _find_zone_id(self, domain: str) -> str:
            ...
            try:
                ...
                if code == 6003:
                    hint = (
                        ...
                        "you'll need the python package cloudflare>=2.3.1.{}".format(
                            " This certbot is running cloudflare "
                            + str(cloudflare.__version__)
                            if hasattr(cloudflare, "__version__")
                            else ""
                        )
                    )
                elif code == 9103:
                ...
                if hint:
                    raise errors.PluginError(
                        ...
                        "that you have supplied valid Cloudflare API credentials. ({2})".format(
                            code, msg, hint
                        )
                    )
                else:
                    logger.debug(
                        ...
                        "Continuing with next zone guess...",
                        e,
                        e,
                    )
        ...
        if msg is not None:
            if "com.cloudflare.api.account.zone.list" in msg:
                raise errors.PluginError(
                    "Unable to determine zone_id for {0} using zone names: "
                ...
                raise errors.PluginError(
                    "Unable to determine zone_id for {0} using zone names: "
                    "{1}. The error from Cloudflare was: {2} {3}.".format(
                        domain, zone_name_guesses, code, msg
                    )
                )
        else:
            raise errors.PluginError(
                ...
                "supplied Cloudflare account.".format(domain, zone_name_guesses)
            )
